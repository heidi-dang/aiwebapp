...
async function exchangeGitHubCode(args) {
...
function validateReturnTo(uiOrigin, returnTo) {
    if (!returnTo)
        return null;
    let url;
    try {
...
export default function createAuthRoutes(store) {
    const auth = createUserAuthService(store);
    return async function registerAuthRoutes(app) {
        app.post('/register', async (req, reply) => {
            const parsed = registerSchema.safeParse(req.body);
            if (!parsed.success)
                return reply.code(400).send({ error: 'Invalid registration data', details: parsed.error.errors });
            try {
        ...
        app.post('/login', async (req, reply) => {
            ...
            if (!parsed.success)
                return reply.code(400).send({ error: 'Invalid login data', details: parsed.error.errors });
            try {
        ...
        app.get('/oauth/:provider/start', async (req, reply) => {
            ...
            if (!uiOrigin)
                return reply.code(400).send({ error: 'Missing ui_origin' });
            ...
            if (provider === 'microsoft') {
                ...
                if (!clientId)
                    return reply.code(500).send({ error: 'Microsoft OAuth not configured' });
            ...
            try {
                ...
                if (provider === 'github') {
                    ...
                    if (!userinfo?.sub || !userinfo?.email)
                        throw new Error('Google userinfo missing fields');
                    profile = {
                    ...
                    if (!me?.id || !email)
                        throw new Error('Microsoft profile missing fields');
                    profile = {
                        id: String(me.id),
                        email,
                    ...
                    const data = (await token.json().catch(() => null));
                    if (!token.ok || !data?.id_token)
                        throw new Error('Apple token exchange failed');
                    const decoded = jwt.decode(data.id_token);
                    if (!decoded?.sub || !decoded?.email)
                        throw new Error('Apple id_token missing fields');
                    profile = {
