...
export async function registerSessionRoutes(app: FastifyInstance, store: Store) {
  app.get('/sessions', async (req, reply) => {
    requireOptionalBearerAuth(req, reply)
    if (reply.sent) return

    const { type, component_id, db_id } = req.query as Record<string, string>
  ...
  app.get('/sessions/:sessionId/runs', async (req, reply) => {
    ...
    const { type, db_id } = req.query as Record<string, string>
  ...
  app.delete('/sessions/:sessionId', async (req, reply) => {
    ...
    const { sessionId } = req.params as { sessionId: string }
    const { db_id } = req.query as Record<string, string>
  ...
  app.delete('/v1//teams/:teamId/sessions/:sessionId', async (req, reply) => {
    ...
    const { teamId, sessionId } = req.params as { teamId: string; sessionId: string }
...
async function findComponentIdForSession(
  store: Store,
  args: { dbId: string; entityType: EntityType; sessionId: string }
): Promise<string | null> {
  ...
  for (const entity of entityList) {
    ...
    if (sessions.some((s) => s.session_id === args.sessionId)) return componentId
  }
  return null
}

async function tryDeleteSession(store: Store, args: { dbId: string; sessionId: string }): Promise<boolean> {
