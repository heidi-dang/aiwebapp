...
If you are running this UI together with the local MVP backend in this repo:

- Backend default: `http://localhost:7777`
- UI default: `http://localhost:3000`

UI production run:

- Copy `ui/.env.example` to `ui/.env.local` (optional)
- Run:
  - `npm run build`
  - `npm start`

## Local MVP backend compatibility notes

If you are using the minimal local backend in this repo (`/server`), it includes a compatibility shim for a legacy UI path typo for deleting team sessions.

- The UI may request:
  - `DELETE /v1//teams/:teamId/sessions/:sessionId`
- Some clients normalize multiple slashes, so the backend also accepts:
  - `DELETE /v1/teams/:teamId/sessions/:sessionId`

This is intended for MVP compatibility; we can later fix the UI and remove the legacy route.

## Note on Next.js workspace root warning

If you see a Next.js warning about an inferred workspace root (commonly caused by multiple lockfiles in parent directories), it is usually safe to ignore during local development. If you run into build or tracing issues, set `outputFileTracingRoot` in `next.config.ts` or remove the extra lockfile.
