...
async function emitEvent(ctx, type, data) {
    const event = {
        id: randomId('evt'),
        type,
        ts: nowIso(),
...
async function handleRunCommandTool(ctx, command) {
 ...
 */
async function simulateTool(ctx, toolName, toolInput, outputChunks) {
    if (ctx.aborted)
        return;
 ...
 */
async function executeWithBridge(ctx) {
    if (!ctx.bridge) {
 ...
 */
async function executeSimulated(ctx) {
...
async function executeWithCopilotApi(ctx) {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        try {
            ...
            if (debug)
                console.log('[CopilotAPI] fetch threw:', msg);
            await emitEvent(ctx, 'error', {
        ...
        for (const call of toolCalls) {
            ...
            if (call.function?.name === 'read_file') {
                ...
                if (!command) {
                    ...
                    function matchesAllowlist(cmd, allowed) {
